...
int ReadNextCommand = 0;
int buf0Length;
int buf1Length;
int buf2Length;

void loop() {
  if (ReadNextCommand) { // if you recieved a N1 Command read the next line of G code and play it simutainously
    ReadNextCommand = 0;
    SDReadGcodeLine();
  }
  GCodeControl();
  if (ResetBuffer[0]) { // G-code buffers need to be reset after recieving a full command
    buf0Length = 0;
    ResetBuffer[0] = 0;
  }
  if (ResetBuffer[1]) {
  ...
  //delay(1000);
  else if (playbackFromSD) { // if no command in the cue and set to read from SD card, grab the next command from the SD
    //delay(250);
  ...
  if (digitalRead(8) != Pin8LastState) {
    myFile.close(); // Viewer Program
    myFile = SD.open("START.TXT"); // Viewer Program
    Pin8LastState = digitalRead(8);
    playbackFromSD = 1;
  }
...
bool CheckRotation() {
  ...
  for (int Joint = 0; Joint < NumJoints; Joint++) {
    if (J[Joint].RotIP) {
      ...
      J[Joint].RotCmdStep = 0;
    }
  }
  if (RotateFlag) {
    for (int Joint = 0; Joint < NumJoints; Joint ++) {
      TimeAndExecuteRotation(Joint);
    }
  }
  return RotateFlag;
}
